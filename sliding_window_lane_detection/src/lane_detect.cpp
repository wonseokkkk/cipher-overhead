...
//  ImageSubscriber_ = this->create_subscription<sensor_msgs::msg::Image>("decrypted_image", qos, std::bind(&LaneDetector::ImageSubCallback, this, std::placeholders::_1));
  ...
  csv_file_.open("/home/avees/ros2_ws/file/framerate.csv");
        if (csv_file_.is_open()) {
            csv_file_ << "Timestamp, FPS" << std::endl; // CSV 헤더 작성
        } else {
            RCLCPP_ERROR(this->get_logger(), "Failed to open CSV file.");
        }

  csv_file_ << "Timestamp, FPS" << std::endl;
...
//  cv::Mat binary_frame;
  ...
  gettimeofday(&endTime, NULL);
  diffTime = (endTime.tv_sec - startTime.tv_sec) + (endTime.tv_usec - startTime.tv_usec) / 1000000.0;
  double fps = 1.0 / diffTime;
  startTime = endTime;

  // Display frame rate on the image
  std::string fps_text = "FPS: " + std::to_string(static_cast<int>(fps));
  putText(new_frame, fps_text, Point(10, 30), FONT_HERSHEY_SIMPLEX, 1, Scalar(0, 255, 0), 2);
